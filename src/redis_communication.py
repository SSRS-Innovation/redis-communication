...
class RedisClient:
    ...
    def get_stream_message_at_time(
            self,
            stream_name: str,
        ...
        try:
            ...
            if search_forward:
                raw_message = self._r.xrange(stream_name, time_in_milliseconds, count=1)
            else:
                raw_message = self._r.xrevrange(stream_name, time_in_milliseconds, count=1)
            # return None if no messages
            if len(raw_message) == 0:
                return None, None
            # latest message
            ...
            self.connect_to_server(self.host, self.port)
            return self.get_stream_message_at_time(stream_name, timestamp, search_forward)
